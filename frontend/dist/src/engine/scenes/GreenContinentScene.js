...
import { greenContinent } from '../../data/greenContinent.js';
...
export class GreenContinentScene extends Phaser.Scene {
  constructor() { super('GreenContinent'); }
  create() {
    this.level = greenContinent; this.physics.world.setBounds(0, 0, this.level.width, this.level.height);
    ...
    this.hero = this.factory.spawnHero(this.level.spawn); this.player = new PlayerController(this, this.hero);
  ...
  createLighting() { this.lights.enable().setAmbientColor(0x5d6e86); this.heroLight = this.lights.addLight(this.hero.x, this.hero.y, 160, 0x9ff3ff, 0.85); this.level.interactives.filter((i) => i.kind !== 'waterfall').forEach((i) => this.lights.addLight(i.x, i.y, 190, i.kind === 'crystal' ? 0x8ff4ff : 0xff8f54, 0.75)); }
